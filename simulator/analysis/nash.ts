...
export class NashEquilibriumSolver {
  ...
  private solveSmallGame(payoffMatrix: number[][]): {
    ...
    if (m === 2 && n === 2) {
      ...
      const player1 = [
        ...
        Math.max(0, Math.min(1, 1 - p)),
      ]
      const player2 = [
        Math.max(0, Math.min(1, q)),
        Math.max(0, Math.min(1, 1 - q)),
      ]
  ...
  private fictitiousPlay(payoffMatrix: number[][]): {
    ...
    while (!converged && iterations < this.maxIterations) {
      ...
      const diff1 = Math.max(
        ...player1Strategy.map((v, i) => Math.abs(v - oldStrategy1[i]))
      )
      const diff2 = Math.max(
        ...player2Strategy.map((v, j) => Math.abs(v - oldStrategy2[j]))
      )
  ...
  analyzeEmpiricalEquilibrium(
    simulations: SimulationResult[]
  ): Map<string, NashStrategy[]> {
    ...
    simulations.forEach((sim) => {
      sim.actions.forEach((action) => {
        ...
        strategies.set(
          action.actionType,
          (strategies.get(action.actionType) || 0) + 1
        )
  ...
  findActionEquilibrium(simulations: SimulationResult[]): {
    ...
    const description =
      `Nash equilibrium suggests playing ${dominantStrategy.actionType} ` +
