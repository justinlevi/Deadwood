...
export class MonteCarloTreeSearch {
  ...
  constructor(explorationConstant?: number, maxIterations?: number) {
    if (explorationConstant !== undefined) this.explorationConstant = explorationConstant
  ...
  private select(node: MCTSNode): MCTSNode {
    ...
    if (current.untriedActions.length > 0 && depth < this.maxDepth) {
      const actionIndex = Math.floor(Math.random() * current.untriedActions.length)
  ...
  private getPossibleActions(state: GameState, playerIndex: number): PendingAction[] {
    const player = state.players[playerIndex]
...
export const mctsAI = (state: GameState, playerIndex: number): PendingAction[] => {
