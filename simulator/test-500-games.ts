...
async function run500Games() {
  ...
  for (let i = 0; i < 500; i++) {
    if (i % 50 === 0) {
      process.stdout.write(
        `\rProgress: ${i}/500 games (${Math.round((i / 500) * 100)}%)`
      )
    }
  ...
  console.log(
    `Average time per game: ${((duration / 500) * 1000).toFixed(0)}ms\n`
  )
  ...
  console.log(
    `  Average Game Length: ${results.reduce((sum, r) => sum + r.rounds, 0) / results.length} rounds`
  )
