...
async function main() {
  ...
  switch (command) {
    ...
    case 'custom': {
      ...
      console.log(`Running custom simulation: ${games} games, ${players} players, ${strategy} AI`)
    ...
    default: {
      ...
      console.log('  full               Run full test suite (1000 games per config)')
...
async function generateQuickReport(analyzer: StatisticsAnalyzer, simulations: any[]): Promise<string> {
...
async function generateFullReport(analyzer: StatisticsAnalyzer, simulations: any[]) {
...
async function verifySimulator(simulator: GameSimulator, analyzer: StatisticsAnalyzer) {
  ...
  for (const strategy of strategies) {
    try {
      ...
      console.log(`   ${strategy.padEnd(12)} ✅ Claims per game: ${claimStats?.avgPerGame.toFixed(1) || 0}`)
    } catch (error) {
  ...
  for (let i = 0; i < 100; i++) {
    results.push(await simulator.simulateGame({
      playerCount: 4,
      aiDifficulty: 'medium', 
  ...
  console.log(`   ✅ 100 games in ${duration.toFixed(1)}s (${(100/duration).toFixed(0)} games/sec)`)
