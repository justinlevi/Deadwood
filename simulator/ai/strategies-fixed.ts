...
export const balancedAI = (state: GameState, playerIndex: number): PendingAction[] => {
  ...
  if (maxClaim > 0) {
    for (let amount = 1; amount <= Math.min(2, maxClaim); amount++) {
      const score = amount * 20 // DOUBLED from 10 to prioritize claims
  ...
  const hasClaimAction = scoredActions.slice(0, 2).some(sa => sa.action.type === ActionType.CLAIM)
  if (!hasClaimAction && maxClaim > 0) {
    // Force a claim as the first action
    actions.push({ type: ActionType.CLAIM, amount: Math.min(2, maxClaim) })
    // Add the highest scoring non-claim action
    const nonClaimAction = scoredActions.find(sa => sa.action.type !== ActionType.CLAIM)
    if (nonClaimAction) {
...
export const aggressiveAI = (state: GameState, playerIndex: number): PendingAction[] => {
  ...
  if (player.gold >= getChallengeCost(player)) {
    ...
    if (challengeTargets.length > 0) {
      ...
      actions.push({ type: ActionType.CHALLENGE, target: challengeTargets[0].index })
    }
  }
...
export const defensiveAI = (state: GameState, playerIndex: number): PendingAction[] => {
  ...
  if (locationSafe && maxClaim > 0) {
    ...
    if (maxClaim > 2 && player.gold > 2) {
      actions.push({ type: ActionType.CLAIM, amount: Math.min(2, maxClaim - 2) })
    }
  }
    ...
    for (const targetPos of edgeLocations) {
      if (targetPos !== player.position) {
        ...
        if (player.gold >= cost) {
          ...
          Object.entries(targetLocation.influences).forEach(([playerId, influence]) => {
            if (playerId !== player.id && influence > 0) {
              targetSafe = false
            }
          })
