...
export const balancedAI = (
  ...
  if (maxClaim > 0) {
    for (let amount = 1; amount <= Math.min(2, maxClaim); amount++) {
      const score =
        amount * 10 + // Base score for influence
...
export const aggressiveAI = (
  ...
  if (player.gold >= challengeCost) {
    ...
    for (let i = 0; i < Math.min(2, challengeTargets.length); i++) {
      if (player.gold >= challengeCost * (i + 1)) {
        actions.push({
          type: ActionType.CHALLENGE,
          target: challengeTargets[i].index,
        })
      }
    }
  }
  ...
  if (actions.length < 2) {
    ...
    if (
      centralLocations.includes(player.position) &&
      currentInfluence < 3 &&
      player.gold > 0
    ) {
...
export const defensiveAI = (
  ...
  // Analyze threats
  const threatsAtLocation = state.players.filter(
    (p, i) =>
      i !== playerIndex &&
  ...
  if (threatsAtLocation > 0 && actions.length < 2) {
    ...
    for (let i = 0; i < LOCATIONS.length; i++) {
      if (i !== player.position) {
        const cost = getMoveCost(player, player.position, i)
        if (player.gold >= cost) {
          const threatsAtTarget = state.players.filter(
            (p, idx) =>
              idx !== playerIndex &&
              (p.position === i ||
