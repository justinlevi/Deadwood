...
export class InMemoryDatabase implements Database {
  ...
  async getSimulations(limit?: number, offset: number = 0): Promise<SimulationResult[]> {
    if (limit) {
  ...
  async getSimulationsByStrategy(strategy: string): Promise<SimulationResult[]> {
    return this.simulations.filter(sim => 
      sim.aiStrategies.includes(strategy)
    )
  }
  
  async getSimulationsByCharacter(character: string): Promise<SimulationResult[]> {
    return this.simulations.filter(sim => 
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('simulations')) {
          ...
          store.createIndex('winnerCharacter', 'winnerCharacter', { unique: false })
