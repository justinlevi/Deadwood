  ...
  switch (command) {
    case 'quick': {
      ...
      winRates.forEach(stat => {
        console.log(`  ${stat.character}: ${stat.winRate.toFixed(1)}% (${stat.wins}/${stat.games})`)
      })
      break
    }
    ...
    case 'export': {
      ...
      console.log(`✅ Exported ${simulations.length} simulations to ${filename}`)
      break
    }
...
function generateReport(analyzer: StatisticsAnalyzer, simulations: any[]) {
  ...
  if (strategies.length > 0) {
    ...
    strategies.forEach(stat => {
      console.log(`  ${stat.strategy.padEnd(12)} ${stat.winRate.toFixed(1)}% win rate`)
    })
  }
}
